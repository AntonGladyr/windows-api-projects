...
void __cdecl main()
{
	...
	/* Init console window */
    	...
    	if ( hEventData == NULL ) {
    		MessageBox(0, "Open first event failed", szEventDataName, MB_OK | MB_ICONERROR | MB_APPLMODAL);
    	...
    	if ( hEventCancel == NULL ) {
    		...
    		MessageBox(0, "Open second event failed", szEventCancelName, MB_OK | MB_ICONERROR | MB_APPLMODAL);
	...
	if ( hMutex == NULL ) {
		...
		CloseHandle( hEventData );
		MessageBox(0, "Open mutex failed", szMutexName, MB_OK | MB_ICONERROR | MB_APPLMODAL);
	...
	if ( hFileMap == NULL ) {
		...
		CloseHandle( hEventCancel );
		CloseHandle( hEventData );
		MessageBox(0, "Could not open file mapping object", szMMFName, MB_OK | MB_ICONERROR | MB_APPLMODAL);
	...
	if ( hFileMem == NULL ) {
		...
		MessageBox(0, "Could not map view of file", "MapViewOfFile", MB_OK | MB_ICONERROR | MB_APPLMODAL);
	...
	hSemaphore = OpenSemaphore(
		...
		CloseHandle( hEventCancel );
		CloseHandle( hEventData );
		MessageBox(0, "Open semaphore failed", szSemaphoreName, MB_OK | MB_ICONERROR | MB_APPLMODAL);
