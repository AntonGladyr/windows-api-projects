...
void __cdecl main( int argc, char *argv[] )
{
	...
	hEventData[1] = CreateEvent(
    	...
    	if ( hEventData[1] == NULL ) {
    		MessageBox(0, "CreateEvent failed", szEventCancelName, MB_OK | MB_ICONERROR | MB_APPLMODAL);  
		//printf( "CreateEvent failed (%d)\n", GetLastError() );
	...
	// Create second auto-reset event object.
	 /*hEventCancel = CreateEvent(
    	...
    	if ( hSemaphore == NULL ) { 
    		MessageBox(0, "CreateSemaphore failed", szSemaphoreName, MB_OK | MB_ICONERROR | MB_APPLMODAL);
	...
	hMutex = CreateMutex(
    	...
    	if ( hMutex == NULL ) {
    		MessageBox(0, "CreateMutex failed", szMutexName, MB_OK | MB_ICONERROR | MB_APPLMODAL); 
	...
	if ( hFileMap == NULL ) {
		MessageBox(0, "Could not create file mapping object", szMMFName, MB_OK | MB_ICONERROR | MB_APPLMODAL); 
